...
ADMIN_PASSWORD = "admin123"   # 先寫死一組帳密，之後想改再改
...
def profile():
    ...
    # ==== 秒殺活動紀錄：user:{user_id}:seckill_orders ====
...
def seckill_join():
    """處理使用者秒殺嘗試（多商品版本），直接使用目前登入的 user。"""
    user_id, resp = require_user()
    if resp:
        return resp
...
def admin_logout():
    ...
    return redirect(url_for("login"))   # 你也可以改成回商品列表等
